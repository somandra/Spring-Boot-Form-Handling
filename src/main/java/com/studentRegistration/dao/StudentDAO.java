...
//String sql="insert into world.student(firstName,lastName,sex,dob,email,section,country,firstAttempt,subjects) values('"+p.getFirstName()+"','"+p.getLastName()+"','"+p.getSex()+"','"+p.getDob()+"','"+p.getEmail()+"','"+p.isFirstAttempt()+"','"+p.getCountry()+"',true,'"+convertListToDelimitedString(p.getSubjects())+"')"; 
		...
		String sql="insert into student(firstName,lastName,sex,dob,email,section,country,firstAttempt,subjects,department) values('"+p.getFirstName()+"','"+p.getLastName()+"','"+p.getSex()+"','"+ConvertDate(p.getDob())+"','"+p.getEmail()+"','"+p.getSection()+"','"+p.getCountry()+"',"+p.isFirstAttempt()+",'"+convertListToDelimitedString(p.getSubjects())+"','"+p.getDepartment()+"')";
	...
	public List<Student> getAllStudents() {
		...
		return template.query("select * from student",new ResultSetExtractor<List<Student>>(){  
		     ...
		     public List<Student> extractData(ResultSet rs) throws SQLException,  
		        ...
		        e.setDepartment(rs.getString(11));
	...
	public void update(Student p) {
		String sql="update Student set firstName='"+p.getFirstName()+"',lastName='"+p.getLastName()+"',sex='"+p.getSex()+"',dob='"+ConvertDate(p.getDob())+"',email='"+p.getEmail()+"',section='"+p.getSection()+"' ,country='"+p.getCountry()+"', firstAttempt="+p.isFirstAttempt()+",subjects='"+convertListToDelimitedString(p.getSubjects())+"' where ID="+p.getId()+"' where department="+p.getDepartment()+"";
		System.out.println(sql);
       template.update(sql);  
	}
	...
	public List<Student> getStudentByDept(String department) {
		// TODO Auto-generated method stub
		return template.query("select * from student where department="+"'"+department+"'",new ResultSetExtractor<List<Student>>(){  
		...
		public List<Student> getStudentByGen(String sex) {
			// TODO Auto-generated method stub
			return template.query("select * from student where sex="+"'"+sex+"'",new ResultSetExtractor<List<Student>>(){  
